...
async def analyze_repo(session: aiohttp.ClientSession, repo_id: int, name: str, description: str, domain_tags: str) -> dict:
...
async def main():
    ...
    cursor = conn.cursor()
    cursor.execute("""
        SELECT id, name, description, domain_tags
    ...
    total = len(rows)
    logger.info(f"待标注仓库数: {total}")

    if total == 0:
        logger.info("没有需要标注的仓库")
        return
    ...
    async with aiohttp.ClientSession() as session:
        tasks = []
        for row in rows:
            repo_id, name, description, domain_tags = row
            task = analyze_repo(session, repo_id, name, description, domain_tags)
            tasks.append(task)
