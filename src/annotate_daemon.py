#!/usr/bin/env python3
"""
ROSClaw LLM Annotator Daemon
后台持续标注，自动处理crawler冲突
"""
...
CRAWLER_PID = 503963  # continuous_crawler.py PID
...
def main():
    log("=" * 60)
    log("🤖 ROSClaw LLM Annotator Daemon Started")
    log("=" * 60)
        ...
        for i, repo in enumerate(repos):
            ...
            if result:
                try:
                    ...
                    log(f"DB update error for {repo['name']}: {str(e)[:50]}")
            else:
